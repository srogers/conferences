module ApplicationHelper
  ...
  def location(thing, options={})
    return Publication::UNSPECIFIED unless thing.present? && thing.respond_to?(:venue) && (thing.venue.present? || thing.location.present?)
    return thing.venue if [Conference::VIRTUAL, Conference::MULTIPLE].include? thing.venue

    elements = []
    if thing.venue.blank?
      elements << ["#{ Publication::UNSPECIFIED } venue"]
    else
